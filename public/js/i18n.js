...
(function (root, factory) {
  ...
  const DICT = {
    en: {
      ...
      "任务交给本机这个 CLI 跑，用的是它的登录态和它的模型，": "Tasks run through this local CLI with its own login and model,",
      "。下面这些 API 模型这会儿一个都用不上，所以先不列了。": ". None of the API models below are used right now, so they're hidden.",
