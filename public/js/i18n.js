...
(function (root, factory) {
  ...
  const DICT = {
    en: {
      ...
      "这块是整台服务器共用的，归平台管理员记。": "This is shared by the whole server, so the platform admin owns it.",
      ...
      "资料库是整台服务器共用的一份，往里放东西归平台管理员": "One library is shared by the whole server; only a platform admin can put things into it",
      ...
      "还没有共享资料。这一块归平台管理员放，放进来之后你的任务也读得到。": "No shared material yet. A platform admin puts things here; once they are in, your tasks can read them too.",
  ...
  const PATTERNS = {
    en: [
      ...
      [/^删掉「(.+)」？$/, "Delete \u201c$1\u201d?"],
      [/^它里面还有 (\d+) 样东西。资料库是整台服务器共用的一份，非空的文件夹删不了——先把里面清空。$/,
       "It still has $1 thing(s) in it. One library is shared by the whole server, so a non-empty folder can't be deleted — empty it first."],
