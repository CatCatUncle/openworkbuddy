...
const TASKS = [
  {
    id: "js-func",
    name: "写函数并自测（代码）",
    rubric: "函数正确（含 n=0/1 边界）、迭代实现、CommonJS 导出规范，且智能体真的自己运行验证过。没自测或用了递归要扣分。",
  ...
  },
  {
    id: "csv-sum",
    name: "CSV 分类合计（数据）",
    rubric: "三个品类合计精确（水果 361 / 蔬菜 209 / 饮料 133，含小数），JSON 值是 number 不是字符串。数字错或类型错都算没达标。",
  ...
  },
  {
    id: "fact-find",
    name: "多文件检索回答（事实）",
    rubric: "必须真读了五个文件再回答：3 个，点名 note1/note3/note5（note5 说的是苹果公司也算「提到苹果」）。数错、漏点名、含糊其辞都扣分。",
  ...
  },
  {
    id: "md-report",
    name: "结构化写作（文档）",
    rubric: "结构达标之外看内容质量：建议要具体可执行（工具名、做法、频率），不许空话凑字。车轱辘话、正确的废话多就往下压分。",
  ...
  },
  {
    id: "html-app",
    name: "单文件网页应用（前端）",
    rubric: "功能真实可用：添加、勾选完成、删除、localStorage 持久化全都实现；界面整洁。只有壳没逻辑、刷新丢数据是硬伤。",
  ...
  },
  {
    id: "fix-bug",
    name: "修 bug 不改测试（调试）",
    rubric: "改动应当最小（核心是循环边界 i < b → i <= b），测试真跑出 PASS，测试文件一字未动。整篇重写、没运行测试都要扣分。",
  ...
  },
  {
    ...
    rubric: "stats.py 逻辑正确、被真实运行过（stats_out.txt 是运行产物）、平均值 24。只写代码不运行是硬伤。",
  ...
  },
  {
    id: "multi-step",
    name: "读数→算→出两份成果（链路）",
    rubric: "三份成果齐全且数据正确（冠军=水果 361）；top.html 展示醒目、summary.md 总结有信息量而不是复读数字。漏一环或数据错要重扣。",
  ...
  },
  {
    ...
    rubric: "总营业额精确 276.5——考点是带引号逗号字段的 CSV 解析。其它数字说明用裸 split 劈开了引号字段，重扣。",
  ...
  },
  {
    id: "strict-format",
    name: "严格输出格式（指令遵从）",
    rubric: "dates.json 恰好是 4 个 YYYY-MM-DD 字符串的升序数组，无多余包装。格式走样（包对象、带注释、顺序错、格式没统一）都算没遵从指令。",
  ...
  },
  {
    id: "log-needle",
    name: "大文件捞针（检索定位）",
    rubric: "行号 217、错误码 EPAY_4417 都答对，且看得出是真检索到的（如引用了那行日志）。编造、只对一半都重扣。",
  ...
  },
  {
    ...
    rubric: "图必须由 gen_diagram 真渲染（SVG 是 graphviz 输出的结构），五个节点齐全、箭头方向对、Redis 是旁挂而不是串在主链路里。手写 SVG、漏节点、连线错都扣分。",
    checks(dir) {
