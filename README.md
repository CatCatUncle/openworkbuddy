...
- **09-23** **对话里的预览卡和右边预览面板打开的不是同一张图**：同一个路径 agent 改了三版，卡片是改完那一刻抓的，右边面板点开是浏览器缓存里的上一版，用户以为它骗人。现在每张卡、每次预览都带那一版的时间戳，服务端只对带版本号的请求给长缓存，点哪张开哪张
- **09-23** **做完老是 `open` 一下替你把文件打开**：`open` / `xdg-open` / `start` / `explorer` 进了「先问」那张表，全自动档也弹一次确认，你明确说「打开」才开；对话里本来就会自动出预览卡，交付只报路径
- **09-23** **记忆超预算时，「交付别替我打开文件」这种规矩被挤掉了**——装不下时按跟本次任务的相关度挑，对事实是对的，对规矩是错的：它跟「写公众号推文」一个词都不沾，比它新的杂事都排在它前面。现在规矩类先进门，剩下的预算再按相关度挑事实；规矩多到装不下也明说，不悄悄丢
- **09-23** **加载过的技能，历史一压缩就忘了，下一轮续跑更是从头没这回事**：`use_skill` 的全文只是一条工具结果躺在历史里，压缩、截短第一个牺牲它，于是同一场对话里第三篇推文又不按技能来了。现在技能全文挂在系统提示词「已加载技能」一节，每一步都看得见，压缩摘要里记一行，续跑自动带回来
- **09-23** **历史压缩那一步用的是全局默认模型，不是这场对话选的那个**：默认模型填成判断模型时压缩直接 400，报错一行看不懂。现在摘要跟着这场对话的模型走；渠道里把判断模型当对话模型用，报错直说它只会挑选项不会写字，去哪儿换
