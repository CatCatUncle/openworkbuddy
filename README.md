...
- **09-17** 成片能垫配乐了：画布上放一个「声音」节点、用途写「配乐」，合成时多一个勾。一说话音乐自动压低，台词一分贝不小
- **09-17** 授权说清楚了：装的这份就是全部功能，买商业授权买的是「可以拿它赚钱」，不是解锁什么；部署配置、CI、脚本、评测集另按 MIT，随便拿
- **09-17** 终端里它也会反问你一句了：「这份报告交 Word 还是 PDF」；`openworkbuddy -f` 带文件图片一起问，打 `/` 弹命令菜单
- **09-17** 生视频接了五家（通义万相 / Seedance / CogVideoX / 海螺 / 硅基流动），认不准是哪家就不发——按条计费的活儿不能白跑
- **09-17** 粘 Key 多框了个空格或中文标点，保存那一刻就指出是第几个字符，不用等发出去收 401
- **09-17** 首页场景按「你要交出什么」重排；无限画布挪到侧栏上面；回答里 `[调研报告](报告.md)` 这种链接点得开了
...
一个「技能」就是一个目录，里头一份 `skill.md` 写着**给 agent 看的指令**。所以装一个技能的真实含义是——**把陌生人写的指令，接到一个能在你机器上执行命令的东西上**。这跟 `npm install` 不是一回事：npm 包要你 `require` 它才跑，技能是 agent 自己会去读、会照着做的，装完之后没有第二道门。

所以从 GitHub 装、或者自己往框里粘一段，都会先过一遍静态检查（34 条规则），然后**把看到的东西摊开给你**：

```
技能「xxx」有 3 处要你看一眼（不是说它一定有问题，是这几处只有你能判断）：
  · references/setup.md:62  把网上下载的东西直接管道给 shell 执行。下载到的内容随时可以被换掉，
                            你审过的和实际跑的不是一个东西。
...
- **不打分，只分三档。** 分数会让人养成「42 分应该还行」的习惯。这里只有：直接装 / 你看一眼再装 / 默认不装。
- **只有 10 条会真拦**（反弹 shell、`curl | bash`、读 SSH 私钥、抹盘、fork 炸弹、抹执行痕迹、Unicode 双向字符伪装文件名……），其余 24 条一律摊开给人看。**拦下的也能被平台管理员显式强装**，那一下会记进系统日志和技能目录里的 `.install.json`（谁、什么时候、从哪个仓库哪个 commit 装的、放行了哪一条）。不留这个口子的后果是人绕过整个工具——把目录直接拷进 `skills/` 谁也拦不住。
- **尽量写成组合，不写成关键词。** `curl` 本身没问题，`printenv` 本身也没问题；但同一个文件里既读密钥又往外发，凑齐就是外带的完整形状。
- **frontmatter 里的那句 `description` 命中会升一级。** 因为它**每一条任务**都会进系统提示词，不管你这次用不用得上这个技能——写在正文里的注入还要等技能被加载，写在这儿的是常驻。
- **技能是整台机器一份的**，所以装 / 改 / 删只有平台管理员能做。加这道闸之前，公司里任何一个刚注册的同事都能给所有人的 agent 递一份指令。

**这东西能干什么、不能干什么，说清楚：** 它的定位是「装之前把话说清楚」，不是查毒。公开标注集上纯静态规则的检出率大约七成五（真恶意技能能认出 74.9%、能判到「别装」的 60.1%），也就是说**四个里会漏一个**。反过来，拿本仓库自带的 34 个真实技能量过，误拦 2 个（5.9%），两个都是真命中而不是规则写错：一个文档里确实写着 `irm … | iex`，另一个是资讯技能缓存下来的博客正文里有一句在**讲解**这种攻击。同一句话写在指令里是攻击、出现在抓回来的资讯里是新闻，正则分不出这两者——这就是静态检查的天花板，也是为什么强装那个口子必须留着。

**想多一把尺子的话，本机装了它就用。** [toolward](https://github.com/CatCatUncle/toolward) 是同一个作者的另一个项目，专做这件事：37 条规则分六族（提示词注入与工具投毒、供应链、密钥、执行与权限、网络与外传、治理）。你机器上有它，装技能、存连接器的时候就会顺手再扫一遍，两份结论合进同一张清单，哪条是谁说的都标着；合的规矩是**只会更严不会更松**——它说拦就拦，它没说的按原来的判，所以挂上它不可能让本来拦得住的变成放行。**它不在依赖里，也不会替你 `npx` 现拉一个**：`npm i -g toolward` 你自己装过才用得上，没装、崩了、超时了一律当它没跑过，安装流程一个字不变。不自动拉包不是偷懒——一边查供应链一边现拉一个没审过的版本，正是它 TW2xx 那族要拦的事。它跟本项目一样是 PolyForm Noncommercial 1.0.0：个人、教学、学术、公益、政府免费，**公司里用要单独授权**（licensing@aijentra.com，可先试 30 天），所以我们不能替你写进 `package.json`，装不装你定。连接器那边它只提醒、从不拦，而且**你填的 Key 和令牌不出门**——递过去的只有变量名，值全换成 `***`，地址只留到路径，问号后面一律剪掉。

最后这条比上面所有规则都重要：**装技能前自己读一眼 `skill.md`**。它是 Markdown，不是二进制。
