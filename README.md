...
- **09-27** 做视频：说「做个 30 秒宣传片」，填表挑时长、画幅、声音，出配音、动效、合成好的 mp4；要花钱的一步默认不做，你勾了才做
- **09-27** 现成配方：宣传片、产品演示、小红书轮播、多平台分发；网页演示能录屏，公开前自动给本机信息打码
- **09-27** 资料库加「工作区」页，一层层点进文件夹，工作区里的文件全找得到；本回合产出也列得出深层文件
- **09-27** 飞书：mp4 当视频发，太大先压缩；卡片写这次花了多少钱，查不到单价写「单价未知」；IM 发文件出错不带出本机路径
- **09-25** 命令行能打多行：粘贴不自动发，`\` 回车或 `Ctrl+J` 换行，`Ctrl+G` 开编辑器，`Ctrl+R` 搜历史；跑着报用时和 token，Esc 停
- **09-25** 命令行：Plan 出完计划直接挑「开干 / 接着改」；`!命令` 自己跑 shell；终端前没人时审批当场拒，`--allow` 预先放行；规范读到仓库根
- **09-25** 回答里提到的文件，只要工作目录里有就能点开，不再只认这一轮刚生成的
- **09-24** 命令行审批改成 ↑↓ 挑、回车确定；工具调用像 Claude Code 那样 `● Shell(命令)` 下面挂 `└ 输出`
- **09-24** 画布出视频：上游已经收下的那一单出错不再自动重跑，免得扣两次钱；带上卡上写的时长、画幅、分辨率
- **09-24** 画布里对话当场进任务历史、显示运行中；输入框提示 `@` 引用文件、`/` 调用技能
