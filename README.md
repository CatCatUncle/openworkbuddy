...
> [!IMPORTANT]
> **起来第一件事是注册管理员。** 第一个注册的就是**超级管理员**（这个位子每个组织只有一个，只能转让不能增发），之后默认不再允许别人自建账号——空实例挂在公网上，等于谁先访问谁是超管。
...
- **09-19** 跑自动评测**不再动你的真记忆、真偏好、真额度账本**——以前评测和正常使用共用一个数据目录，题目里 agent 顺手 `remember` 一句就真进了你的长期记忆，下一轮评测还被上一轮记下的东西影响。现在整轮痕迹都锁在 `eval/runs/<时间戳>/` 里。题库同时补了三种单轮题考不出来的题形：长任务（规范藏在文件里，跑 40 步以后还守不守）、多轮（口径只在第一轮说一次）、记忆召回（规矩种进记忆，题面一个字不提，另外种两条不相干的看它会不会硬套）。外加一道不花钱的离线闸门：把每道题的判分喂给空目录，一条都不许绿——当场抓出两道老题在「模型什么都没干」时也能得分。
- **09-19** 两条任务同时改一个代码仓库时，**后到的那条自己去分身里改**。工作目录指在自己仓库上的时候，这事以前是危险的：A 刚把函数改成三个参数，B 手里还是两个参数的旧文本，写回去就把 A 的改动抹了，谁也不报错，测试可能还是绿的。现在撞上了就去 git worktree——同一个 `.git`、独立工作区、分支 `owb/<会话>`，**你眼前那份工作区一个字不动**；你手上没提交的改动会一起带过去（不带的话它以为同事的活没干，把你改好的地方再改一遍）。跑完自动提交到那根分支，但绝不自动合回来——合并冲突怎么取舍是它最没资格拍板的事，`git merge` 那句就摆在过程卡上等你按。`openworkbuddy worktree` 看现在有哪些分身。只有撞上才隔离：一个人一条任务的时候什么都没变
- **09-19** **超级管理员**这一档补上了，权限按 RBAC 重做。用户原话：「怎么一个管理员能取消其他管理员或者增加管理员的权限了」——以前确实是：任何一个管理员都能把另一个降成成员、停用、删号，也能随手再造一个管理员。现在四档（超管 / 管理员 / 审计员 / 成员），规矩只有两条：**只能管比自己低的那一档**、**授角色要同时够得着人和角色**。发管理员的四条路（改角色、建号、邀请码、部门模板）堵在同一道闸上；超管每组织一个，只能转让不能增发。锁死了有出口：平台超管跨组织救场，或在服务器上跑 `openworkbuddy owner`
- **09-19** 短剧分镜**能退回上一版了**。改一句台词重跑一下，上一版首帧以前会被同名文件盖掉——光记路径没用，路径没变，字节换人了。所以留底存的是字节本身，按 sha256 去重。点「版本」翻这一镜有过哪些样子，退回去是整镜换回那一版（合并会留下「上一版的视频配这一版的首帧」那种四不像）；退之前先给现在这一版拍一张，退错了还能再退回来
- **09-19** 欠费的渠道**撞一次就断**。用户原话：「一些不能用的模型，不要一直去用啊！」。阿里云百炼把欠费报成 HTTP 400，正文里一个「余额」都没有，只有 `code: Arrearage`，熔断器以前一个词都接不住，于是当成「这次不巧」，下一次照发、再白等一个超时——而这条渠道底下挂的正是出厂那条默认视频模型。现在一次就断，还告诉你是没余额该去充值，不是「等几分钟再试」
- **09-19** 火山方舟上**转售的别家型号不再被当成挂错渠道**。方舟给上架的型号都盖一个 `-YYMMDD` 尾巴，而它转售别家——`kimi-k2-250711` 挂的就是方舟。老判据只看牌子前缀，把用户填对的 `glm-5-3-flash-260828` 认成智谱家的、保存一次就挪走，挪走之后反倒必然报「型号不存在」。现在尾巴比前缀可信，且必须是真日期，别家的 8 位日期写法不许被吃掉
- **09-19** 卡住的任务**自己会停下来了**。以前只有「劝」：同一个调用连着三次拿到同样的结果就提醒换条路，五次拦一次；模型不听就一路转到上限，你看到的是「已达最大运行时间」，还以为活儿太多。现在五种卡法各认各的——同参同结果、一字不差的同一句报错、连续失败、渠道熔断了还硬调，以及 `A→B→A→B` 这种单看每个工具都不重复的来回转圈。这种停不自动续跑，也不劝你调大上限（上限再大它也只是多转几圈），而是点名让你去修它撞墙的那条路
- **09-19** 资料库里**音频视频点开就能听能看**了，一个 1006 KB 的 mp3 不再被判「文件太大」。根子在服务端：取文件一律当附件发，`<audio>`/`<video>` 全渲染不出来，现在默认内联发并带对 MIME，要存盘的走 `?dl=1`。二进制文件只探一下在不在，不整包拉回来。**「出自任务」不再张冠李戴**——按文件自己所在的任务目录认，认不出来就交白卷，不猜一个回去。还多了「所在位置」和「复制文件」两颗按钮：一个在访达里选中它，一个把文件本身放进剪贴板，粘到微信或 PPT 里是文件不是路径
- **09-19** 让它**用真浏览器打开网页看一眼**这条路修通了。以前除了「列出有哪些标签页」，导航、截图、跑 JS 全挂，还得你自己先开一个带调试口的 Chrome。四个毛病叠在一起：9222 端口上蹲着一个不给调试口的 Chrome、握手里带了 `Origin`（Chrome 111 起直接 403）、成功的 101 被当成失败、`evaluate` 多剥一层所以永远是 `undefined`。现在它自己拉一个独立配置的 Chrome，端口交给系统挑，碰不到你正开着的窗口和登录态；能截整页长图、能跑 JS 取数，无头档下 WebGL 也真画得出来。连不上先跑 `status`，只探不碰
- **09-19** 它反问你「三版对比在《封面三选一.html》里，你要哪版」的时候，**这个文件自己摊到右边**。只认题面里点名、而且这一趟真写出来的文件，所以不会弹「文件不存在」；问题下面还挂着这几份文件的按钮。跑完才出产物那条路一个字没改——产出还是不抢版面
- **09-19** 成果文件面板顶上多了个**按名字找**。搜的是整条相对路径，文件夹名也算线索（打「0918」就把那天那个任务夹里的都捞出来）；空格分词、全都命中才算；结果摊平，每行标着它在哪个文件夹。文件不到 8 个不摆这个框，一眼扫得完的时候它纯占地方
- **09-19** 公众号封面**改成真生图**，不再拿 HTML 排版截个图充数。技能里原来只写了尺寸，一个字没说画面该长什么样，于是每次都退回去排版。现在提示词按主体 / 风格 / 光与色 / 构图四行拼，一次出三个方向摆一个对比页让你挑，两个尺寸都交，且不让模型往图上写标题（中文字必糊）
- **09-19** 改文件那几步**能退回去了**：落盘前先留底，过程卡上点开就是这一步的 diff，旁边一颗「回退到这步之前」——那一步和之后动过的文件一起退，退错了再按「撤销回退」。命令行 `/rewind`。「每步都问」时审批条上直接是那几行 diff，批的是改动不是文件名
