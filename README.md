...
这个包没买代码签名证书（苹果一年 99 美元、Windows 一年几千块，这是个免费开源项目），不是有毒。

- **macOS**：上面那句 `curl` 一步到位——浏览器下的文件会被打上隔离标记，curl 下的不会。已经手动下了 dmg 的，拖进「应用程序」后跑 `xattr -dr com.apple.quarantine /Applications/OpenWorkBuddy.app`。（右键「打开」只在 macOS 14 及更早管用，15 起被苹果取消，弹窗里只剩「完成 / 移到废纸篓」——照着老教程点会以为「就是打不开」。）
...
- **09-19** **撞不通的模型不再一趟一趟去撞**：401/403/404 这种一次就停 30 分钟；欠费的渠道撞一次就断，还告诉你是该去充值
- **09-19** 卡住的任务**自己会停下来了**，五种卡法各认各的，并点名让你去修它撞墙的那条路，而不是劝你调大上限
- **09-19** **改哪步都能退回去**：短剧分镜退回上一版，改文件那几步在过程卡上点开就是这一步的 diff，旁边一颗回退
- **09-19** 「用真浏览器打开这个网页看一眼」这条路**修通了**，能截整页长图、能跑 JS 取数，碰不到你正开着的窗口和登录态
- **09-18** 任务历史**能搜对话正文了**，也能按意思搜，每条结果标着是靠标题、产出文件还是意思相近找到的
- **09-17** 任务跑完会问一句：这一趟造的几百张帧图、分出来的音轨、打包产物要不要清掉，**成片和报告一个不碰**
