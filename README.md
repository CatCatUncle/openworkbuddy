...
- **09-18** 终端里 `/resume` 接回之前那段对话，从「数序号」变成「挑」：↑↓ 选、打字搜、回车接上，桌面端开的那些也在同一张单子里；`/model` 同一个选择器。另外补了 `/init`（让它自己看一遍目录，写份 `AGENTS.md`）、`/compact`、`/diff`、`/mcp`，以及 `Shift+Tab` 切权限档、`Esc Esc` 把问过的话拉回来改
