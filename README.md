...
四个入口：命令行 `openworkbuddy jev`、接口 `/api/decide`、渠道卡上那颗认得出它的「测一下」，以及 **agent 手里的 `decide` 工具**——一趟问完 32 道题，低于门槛的那几条它挑出来交给你，不当定论往下走。产品里已经在用两处：目标模式的验收（每条标准一道是非题，过 70% 才打勾），和 **triage 技能**的批量分拣（工单分派、简历初筛、反馈归类）——先拿十来条校判准，再跑批，最后交两份：能直接走的，和要人看的。一道题约两万分之一美金。
