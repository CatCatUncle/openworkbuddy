...
wb -f 季度数据.xlsx "照这张表做份复盘 PPT"  # 带文件/图片一起问，-f 写几次带几个
...
wb completion zsh > ~/.zsh/completions/_wb # Tab 补全（bash / zsh / fish）
```

岔路口上它会**反问你一句**——「报告交 Word 还是 PDF」这种，每个选项底下写清楚选它意味着什么。管道和 cron 里不会问，那种场合它自己拿主意往下做。

直接敲 `wb` 是交互模式：打一个 `/` 就把命令菜单弹出来（`/model` 换这趟活儿谁来干、`/paste` 把剪贴板里的截图带进来、`/open` 打开刚做好的文件），文件也能直接从访达拖进窗口。
...
- **09-17** 终端里它也会反问你一句了：「这份报告交 Word 还是 PDF」；`wb -f` 带文件图片一起问，打 `/` 弹命令菜单
