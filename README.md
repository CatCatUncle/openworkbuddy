...
- **09-21** **英文界面下，每按一次「删掉」弹出来的都是中文**：全站 31 处删除确认走的是原生 `confirm()`，而翻译照着 DOM 走——原生框里那句话从头到尾只是个 JS 字符串，没进过 DOM，扫界面的闸门也发现不了。顺带两笔：浏览器里一旦勾上「不再显示对话框」，`confirm()` 静默返回 false，每颗删除钮当场变哑巴；原生模态框离屏点不动，这 31 条删除路径一条没验过。现在全换成自绘的框，红钮上写动词，焦点落在「算了」上，要删哪几样一条条列出来（清单封高度、自带滚动——顶出屏幕的话连「算了」都按不着）
