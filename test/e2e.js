...
function testPluginManifest() {
  ...
  try {
    // 完整合法插件
    let p = load({ manifest: goodManifest({ version: "1.2.0", license: "MIT", author: { name: "小林" }, keywords: ["a"] }), skills: { alpha: skillBody("alpha"), beta: skillBody("beta") } });
...
async function testMcpStreamableHttp() {
  ...
  try {
    ...
    const r = await client.callTool("ping", { who: "小林" }, 10000);
    assert(!r.isError && r.content === "pong:小林", "工具调用结果不对: " + r.content);
...
function testMemoryLayer() {
  ...
  const script = `
    ...
    mem.add({ text: "公司名叫青禾", shared: true });
