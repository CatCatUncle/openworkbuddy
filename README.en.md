...
- **Sep 23** **The preview card in the chat and the right-hand preview panel were opening different images**: the agent rewrote the same path three times, the card grabbed the moment it finished, the panel served the browser's cached previous version, and it looked like the agent was lying. Every card and every preview now carries that version's timestamp, the server only long-caches requests that name a version, and whichever card you click is the one that opens
- **Sep 23** **It kept running `open` to pop the file up for you once it was done**: `open` / `xdg-open` / `start` / `explorer` are now on the ask list, so even full-auto raises one confirmation and only an explicit "open it" goes through; the chat already shows a preview card, so a delivery just reports the path
- **Sep 23** **When memory ran over budget, a rule like "don't open files for me on delivery" got squeezed out** — over budget, entries are picked by relevance to the task, which is right for facts and wrong for rules: that one shares no word with "write a WeChat article", so every newer bit of trivia outranked it. Rules now go in first, the remaining budget picks facts by relevance, and too many rules to fit is said out loud rather than dropped silently
- **Sep 23** **A loaded skill was forgotten the moment history got compacted, and the next turn never knew it had existed**: the full text from `use_skill` was just a tool result lying in the history, the first thing compaction and trimming sacrifice, so the third article in the same conversation ignored the skill again. Loaded skills now sit in a "loaded skills" section of the system prompt, visible at every step, noted in the compaction summary and brought back on the next turn
- **Sep 23** **Compaction used the global default model, not the one this conversation picked**: with a decisions model set as the default, compaction failed with an opaque 400. The summary now follows the conversation's model, and a decisions model used as a chat model gets a plain error saying it only picks options, never writes, and where to change it
- **Sep 22** **Type `/`, pick a skill, press Enter — and the message flew off** before you had written what you wanted done. The `/` listener only called `preventDefault()`, which suppresses the newline, not the send listener sitting on the same input; **what needed stopping was propagation, not the default action**. ↑↓ now pick a row too (selection used to be pinned to the first one, so the keyboard could not choose at all). **A newly installed skill needing a page reload before `/` could find it** is fixed in the same pass: that list was fetched once at page open, and now refreshes when it is about to be used — hub, plugin bundle or a file dropped in from the command line, all count
- **Sep 22** **On a phone you had to jab at that row of buttons two or three times to hit one**: measured at 390 wide, 9 of 50 clickable things had a side under 36px — the send button 32 square, attach, sidebar and the model picker all 30 tall. A fingertip contact patch is 8–10mm, about 40px on a 390-wide screen. **That is not “a bit small” — it is the same action working sometimes and not others**, and you cannot tell whether you missed or the app ignored you. On touch the whole family now goes to 44 (the ones wedged inside rows get 40; the × on a chip spills its hit area out with negative margins — still 18 to the eye, 32 to your finger). **Gated on pointer rather than window width**: a desktop browser dragged narrow is still a mouse, and there is no reason to spend density on it. Measured again afterwards: 0 under 44 on touch, 0px of horizontal overflow, nothing moved on mouse
- **Sep 22** **The desktop pet was ticked on by default, with nothing on the desktop**: three places in the backend agree that an unconfigured pet is off; only the settings card rendered it ticked — you would go and debug why the pet never appeared, not why the checkbox was ticked. **A UI that lies about state is harder to chase than the wrong state itself.** The switch moved out too: it used to sit in a card you had to scroll down to under settings → assistant, and **a switch that exists but cannot be found is no switch at all**. It now sits in the avatar menu next to “language” — On / Off, one click, the menu stays open and flips in place, and flips back in red if the save fails. Pure server mode has no desktop window, so the row is not drawn at all
